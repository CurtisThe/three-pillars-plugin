...
2. **Read project context**: Read `docs/architecture.md`, `docs/product_roadmap.md`, and `docs/known_issues.md` if they exist. Use as project context for the design conversation — do not require them. If the roadmap has a `## Current Focus` table, note where this new design fits relative to current priorities — mention this during the design conversation so the user can decide its priority.
...
4. **Have a design conversation**. Your job is to draw out:
   ...
   - **Constraints** — performance, compatibility, dependencies, resource limits, etc.
   - **Open questions** — things the user isn't sure about yet.
   Ask clarifying questions. Push back on vague requirements. Suggest trade-offs.
5. **Write `docs/tdd-designs/<design-name>/design.md`** with this structure:
