...
Note (retire-approval-tags): the `_auto_strip_after_push` helper (which REST-DELETEd
`tp:human-approved` after a push) has been removed. The review-path approval
(Path B) is self-cleaning — a review's immutable `commit_id` fails closed when
the head advances — so no push-time strip is needed.
"""
