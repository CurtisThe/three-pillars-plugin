...
5. **Read project context**: Read `docs/product_roadmap.md` if it exists. If the roadmap has a `## Current Focus` table, note where this spike fits relative to current priorities — mention this during the spike conversation so the user can decide its priority.
...
6. **Write `docs/tdd-designs/<spike-name>/design.md`** with this structure:
...
9. **Update Current Focus**: If the roadmap has a `## Current Focus` table and the user indicated this spike is a near-term priority during the conversation, propose adding it to the Current Focus table with an appropriate priority, next action (`/tdd-spike-plan`), and any blockers. Show the proposed row and get user confirmation. If the user didn't indicate priority, ask whether it belongs in Current Focus.
10. **Tell the user** the next step is `/tdd-spike-plan <spike-name>` to create an experiment plan.
