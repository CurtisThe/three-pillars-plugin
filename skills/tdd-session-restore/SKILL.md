...
2. **Read `handoff.md`** from the design directory. If it doesn't exist, tell the user there's no prior session to restore. Check if any other artifacts exist (design.md, plan.md) and summarize what's available, then stop.
3. **Read the sibling artifacts** that exist in the design directory (design.md, detailed-design.md, plan.md, review.md) — quick scan for context, not deep-dive.
4. **Read the files mentioned** in the handoff's State and Next sections (just a quick scan — don't deep-dive unless something looks wrong).
5. **Read Current Focus**: If `docs/product_roadmap.md` exists and has a `## Current Focus` table, read it. Use this to contextualize the design's status within the broader project — is it the top priority? Is it blocking other work?
6. **Present a brief status** to the user:
...
7. **Ask** if they want to continue where they left off or pivot to something else.
