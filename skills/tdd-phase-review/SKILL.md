...
1. **Read all design artifacts**: `design.md`, `detailed-design.md`, `plan.md` from the design directory.
2. **For each completed task in plan.md**:
...
3. **Run the full test suite** for all affected files. Discover the test command from the project config (CLAUDE.md, Makefile, package.json, pyproject.toml, etc.):
   ```
...
5. **Write `docs/tdd-designs/<design-name>/review.md`**:
...
6. **Present findings** to the user concisely. If there are gaps, suggest whether to add tasks to the plan or update the design.
