...
Release your ownership of a design so another developer can claim it without `--force-takeover`. This is the graceful handoff path — unlike `/tp-design-complete`, the design is *not* finished; you're just stepping away.
...
## Prerequisites
- `three-pillars-docs/tp-designs/{design-name}/lock.json` must exist. If not, tell the user there's nothing to release and stop.
...
6. **Commit the release** per `skills/_shared/commit-after-work.md`. The lock change IS the substantial work here, so it goes in its own commit. Artifact paths to stage:
   - `three-pillars-docs/tp-designs/{design-name}/lock.json`
...
- Do NOT delete the design directory or any artifacts. This skill only modifies `lock.json`. To archive a finished design, use `/tp-design-complete`.
